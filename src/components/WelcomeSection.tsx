...
const WelcomeSection: React.FC = () => {
  ...
  const budgetSpent = budgetData ? Math.round(totalBudget * 0.24) : 0; // 24% spent as example
  const activeAgencies = budgetData ? 1 : 0;
  ...
  const quickStats = [
    ...
    { 
      icon: CreditCard, 
      label: 'Budget Spent', 
      value: budgetSpent > 0 ? `₹${budgetSpent.toLocaleString('en-IN')} this month` : '₹0 this month', 
      action: 'Track ROI',
      color: 'text-blue-600',
      bgColor: 'bg-blue-50'
    },
