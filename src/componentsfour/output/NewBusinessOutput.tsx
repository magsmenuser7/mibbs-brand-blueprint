...
function generateNewReport(data: QuestionnaireData) {
  ...
  const industryMultiplierMap: Record<string, number> = {
    "Retail & E-commerce": 4,
    "Food & Beverages": 3,
    "Technology & IT": 5,
    "Real Estate": 2.5,
  ...
  const multiplier =
    industryMultiplierMap[data.industry] || (3 + rand() * 2);

  const monthlyRevenue = Math.round((totalBudget * multiplier) / 12);

  const spendPercent = 20 + rand() * 10;

  const monthlyBrandBudget = Math.round(
    monthlyRevenue * (spendPercent / 100)
  );
  ...
  return { ind, totalBudget, categories, channelBars, helpAreas, monthlyRevenue,monthlyBrandBudget,annualBudget, };
}
